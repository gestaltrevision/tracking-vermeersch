...
if __name__ == "__main__":
    ...
    #python process_acc_data.py --main_dir "C:\Users\jeuux\Desktop\Carrera\MoAI\TFM" --parameters_file "C:\Users\jeuux\Desktop\Carrera\MoAI\TFM\AnnotatedData\Accelerometer_Data\Datasets\fc_parameters_basic.json" -d
    ...
    cols=["id","time","AccX","AccY","AccZ"]
    table_arr=[]
    for file in tqdm(table_paths):
        df=pd.read_csv(file)
        #Extract features 
        table_arr.append(create_df_participant(df[cols],fc_parameters,window_duration,sample_rate))
    #concat tables
    complete_table = pd.concat(table_arr)
    #Split train/val
    index_list=np.asarray(complete_table.index)
    ...
    complete_table["train"]=train_val #Create column with "1" if entry is a train entry, 0 otherwise
    complete_table["target"]= np.nan
    
    #split tables
    train_df=complete_table[complete_table["train"]==1]
    train_df=train_df.reset_index()
    test_df=complete_table[complete_table["train"]==0]
